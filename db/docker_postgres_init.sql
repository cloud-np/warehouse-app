...
CREATE TABLE driver(
    id SERIAL PRIMARY KEY,
    dname TEXT NOT NULL,
    -- Idealy we could use a counter to determine how many packages he has left to scan.
    -- But since based on the assignment there is only driver per cluster. So knowing
    -- the number of packages left to scan in the cluster is the same.
...
BEGIN
    ...
    -- This trigger's the other trigger. I should put a constrain here to run
    -- the query and then remove the constrain. But since we are inserting one 
    -- package at the time there would not be a significant problem. Later on this should get reworked.
    UPDATE package p
